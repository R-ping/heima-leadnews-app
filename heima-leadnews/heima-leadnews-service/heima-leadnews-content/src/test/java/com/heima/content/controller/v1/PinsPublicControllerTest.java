...
import com.heima.content.service.PinsPublicService;
...
class PinsPublicControllerTest {
    ...
    @Test
    void testUploadImageOssClientNull() {
        byte[] content = "fake image".getBytes();
        MockMultipartFile file = new MockMultipartFile("file", "test.jpg", "image/jpeg", content);

        // Set ossClient to null
        ReflectionTestUtils.setField(controller, "ossClient", null);
    ...
    @Test
    void testUploadImageOssConfigNull() {
        byte[] content = "fake image".getBytes();
        MockMultipartFile file = new MockMultipartFile("file", "test.jpg", "image/jpeg", content);

        // Set ossConfig to null
        ReflectionTestUtils.setField(controller, "ossConfig", null);

        ResponseResult result = controller.uploadImage(file);

        assertEquals(AppHttpCodeEnum.SERVER_ERROR.getCode(), result.getCode());
        // Restore for other tests
        ReflectionTestUtils.setField(controller, "ossConfig", ossConfig);
    }

    @Test
    void testUploadImageIOException() throws Exception {
        ...
        ResponseResult result = controller.uploadImage(mockFile);

        assertEquals(AppHttpCodeEnum.SERVER_ERROR.getCode(), result.getCode());
    }

    @Test
    void testListInvalidSizeTooSmall() {
        when(pinsPublicService.list(eq("latest"), eq(1), eq(10)))
                .thenReturn(ResponseResult.okResult());

        ResponseResult result = controller.list("latest", 1, 0);

        assertEquals(200, result.getCode());
    }

    @Test
    void testCommentListInvalidSizeTooSmall() {
        when(pinsPublicService.commentList(eq(1L), eq(1), eq(10)))
                .thenReturn(ResponseResult.okResult());

        ResponseResult result = controller.commentList(1L, 1, 0);

        assertEquals(200, result.getCode());
    }

    @Test
    void testTopicsInvalidSizeTooSmall() {
        when(pinsPublicService.topics(eq(null), eq(1), eq(20)))
                .thenReturn(ResponseResult.okResult());

        ResponseResult result = controller.topics(null, 1, 0);

        assertEquals(200, result.getCode());
    }

    @Test
    void testTopicsInvalidSizeTooLarge() {
        when(pinsPublicService.topics(eq(null), eq(1), eq(20)))
                .thenReturn(ResponseResult.okResult());

        ResponseResult result = controller.topics(null, 1, 100);

        assertEquals(200, result.getCode());
    }

    @Test
    void testCommentListInvalidSizeTooLarge() {
        when(pinsPublicService.commentList(eq(1L), eq(1), eq(10)))
                .thenReturn(ResponseResult.okResult());

        ResponseResult result = controller.commentList(1L, 1, 100);
